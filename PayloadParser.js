function parseUplink(device, payload)
{
    ...
    if(parsed.statusLE != null){
        var statusLE = device.endpoints.byAddress(1);

        if (stausLE != null)
            statusLE.updategenericSensor(parsed.statusLE);
    }
    
    if(parsed.statusUPS != null){
        var statusUPS = device.endpoints.byAddress(2);

        if (statusUPS != null)
            statusUPS.updategenericSensor(parsed.statusUPS);
    }
    
    if(parsed.statusSem != null){
        var statusSem = device.endpoints.byAddress(3);

        if (stausSem != null)
            stausSem.updategenericSensor(parsed.statusSem);
    }
        
    if(parsed.statusSis != null){
        var statusSis = device.endpoints.byAddress(4);

        if (statusSis != null)
            statusSis.updategenericSensor(parsed.statusSis);
    }     
}
